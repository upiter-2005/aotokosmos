...
// echo '</pre>';
    if ($catname->parent){
        ...
        //здесь мы можем проставить ссылки
...
<div class="breadcrumbs">
    ...
    if(get_query_var('yearcar')){
        if($category->parent){
            ...
            $current_link = get_term_link($category->cat_ID);
            $if_model = $category->name; 
            echo '<a href='.$link.'>'. $parent->name .'</a> / ';
            echo '<a href='.$current_link.'>'. $category->name .'</a> / ';
...
<h1 class="cat-h1">Автомобили <?php echo $parent_title; ?> <?php echo get_query_var('yearcar'); ?></h1>
    ...
    <section class="brand-search">
		<div class="container-min ">
			<div class="brand-search-wrap years-form">
				...
				<h2 class="ta-left"> <?php echo $termName . ' ' . $if_model; ?> по годам</h2>
				<div class="row">
